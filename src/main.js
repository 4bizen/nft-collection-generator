...
const csvWriter = createCsvWriter({
    path: `${buildDir}/_build.csv`,
    fieldDelimiter: ';',
    alwaysQuote: true,
});
let csvWriterRecords = [];
...
const addMetadata = _edition => {
    ...
    let writeToCSVRecord = [];
    writeToCSVRecord.push(tempMetadata.edition);
    tempMetadata.attributes.forEach(function (item) {
        writeToCSVRecord.push(item.name);
    });
    writeToCSVRecord.push(writeToCSVRecord.join(',') + '.png');
    csvWriterRecords.push(writeToCSVRecord)
...
const createFiles = async edition => {
    ...
    csvWriter.writeRecords(csvWriterRecords);
